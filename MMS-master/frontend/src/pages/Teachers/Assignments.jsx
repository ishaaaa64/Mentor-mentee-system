...
const AssignmentSection = () => {
  const [newAssignment, setNewAssignment] = useState({ title: '', description: '', cardID: '', deadline: '' });
  ...
  const handleAddAssignment = async (e) => {
    e.preventDefault();
    if (newAssignment.title.trim() !== '' && newAssignment.description.trim() !== '' && newAssignment.cardID.trim() !== '' && newAssignment.deadline.trim() !== '') {
      try {
        const response = await axios.post('http://localhost:4000/api/v1/assignments', newAssignment);
        setAssignments([...assignments, response.data.assignment]);
        setNewAssignment({ title: '', description: '', cardID: '', deadline: '' });
      } catch (error) {
      ...
      <Content>
        <AssignmentsContent>
          ...
          <AddAssignmentForm onSubmit={handleAddAssignment}>
            ...
            <AddAssignmentInput
              type="text"
              placeholder="Enter assignment cardID"
              value={newAssignment.cardID}
              onChange={(e) => setNewAssignment({ ...newAssignment, cardID: e.target.value })}
            />
            <AddAssignmentInput
              type="text"
          ...
          <AssignmentList>
            {assignments.map((assignment) => (
              <AssignmentItem key={assignment.id}>
                ...
                {assignment.description}, {assignment.cardID}, {assignment.deadline}
              </AssignmentItem>
            ))}
          </AssignmentList>
        </AssignmentsContent>
