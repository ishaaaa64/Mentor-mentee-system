...
const CheckExamSection = () => {
  ...
  const handleAddExam = async (e) => {
    ...
    const newExam = { name, rollNo, className, marks: parseInt(marks) };
    try {
  ...
  const calculateTotalMarks = () => examData.reduce((total, exam) => total + exam.marks, 0);
        ...
        {examData.length > 0 ? (
          <ul>
            {examData.map((exam, index) => (
              <li key={index}>
                Name: {exam.name}, Registration Number: {exam.rollNo}, Class: {exam.className}, Marks: {exam.marks}
              </li>
            ))}
          </ul>
        ) : (
